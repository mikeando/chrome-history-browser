...
async function getAggregatedHistory(startTime: number, endTime: number, ignored: Set<string>): Promise<Map<string, HistoryNode>> {
  ...
  const rootNodes = new Map<string, HistoryNode>();
...
function createRowElement(node: HistoryNode, depth: number = 0): HTMLElement {
...
function getTimeBounds(): { startTime: number, endTime: number } {
  const preset = (document.getElementById('time-preset') as HTMLSelectElement).value;
  const now = Date.now();
  let startTime = 0;
  let endTime = now;
  ...
  switch (preset) {
    case '1w': startTime = now - (7 * ONE_DAY); break;
    case '1m': startTime = now - (30 * ONE_DAY); break;
    case '3m': startTime = now - (90 * ONE_DAY); break;
    case 'custom':
      const startInput = (document.getElementById('date-start') as HTMLInputElement).value;
      const endInput = (document.getElementById('date-end') as HTMLInputElement).value;
      if (startInput) startTime = new Date(`${startInput}T00:00:00`).getTime();
      if (endInput) endTime = new Date(`${endInput}T23:59:59`).getTime();
      break;
  }
...
function setupTabs() {
  ...
  const toggleViews = (showHistory: boolean) => {
...
async function loadIgnoredDomains() {
  ...
  ignoredDomains = new Set(data.ignoredDomains as string[]);
}
...
document.addEventListener('DOMContentLoaded', async () => {
  const presetSelect = document.getElementById('time-preset') as HTMLSelectElement;
  const customGroup = document.getElementById('custom-date-group') as HTMLDivElement;
